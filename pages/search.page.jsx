...
import { selectSearchResults, selectSearchQuery } from "../redux/search/searchSlice"; 
import { selectFilter } from "../redux/filter/filterSlice"; 
import { applyFilterAndSort } from "../redux/filter/filter.utils"; // 1. Import Utils
...
const SearchResults = () => {
  ...
  // Redux Data
  const baseSearchResults = useSelector(selectSearchResults);
  ...
  // === CLEANER LOGIC ===
  useEffect(() => {
    const filtered = applyFilterAndSort(baseSearchResults, filters);
                ...
                {finalProducts.length > 0 ? (
                    finalProducts.map((item) => (
                        <div key={item.id} className="w-full border border-gray-200 rounded-lg flex flex-col md:flex-row gap-4 p-4 hover:bg-gray-50 transition-colors">
                            <Link to={`/product/${item.id}`} className="w-full md:w-64 flex justify-center items-center bg-gray-100 rounded-md">
                                <img className="h-44 object-contain mix-blend-multiply p-2" src={item.image} alt={item.title} />
                            </Link>
                            
                            <div className="flex-grow flex flex-col gap-1">
                                <Link to={`/product/${item.id}`}>
                                    <h2 className="text-lg font-medium hover:text-[#c7511f] hover:underline cursor-pointer">
                                        {item.title}
                                    </h2>
                                </Link>
                                <div className="flex items-center gap-2 mt-1">
                                    <div className="flex">{renderStars(item.rating?.rate)}</div>
                                ...
                                <div className="mt-2">
                                    ...
                                    <span className="text-xs align-top">{(item.price % 1).toFixed(2).substring(1)}</span>
                                </div>
