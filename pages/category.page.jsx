...
const CategoryPage = () => {
  ...
  // 2. Get Filter State (Whole Object)
  const filters = useSelector(selectFilter);

  // === CLEANER LOGIC ===
  useEffect(() => {
    if (products) {
      // Filter by Category (URL) first
      let categoryProducts = products;
      if (category !== "all") {
        categoryProducts = products.filter((item) => item.category === category);
      }

      // Apply Sidebar Filters (Price, Rating, Sort) using Utils
  ...
  return (
    <div className="w-full bg-white min-h-screen font-bodyFont">
      <div className="max-w-[1500px] mx-auto p-4 flex gap-6">
        ...
        {/* === LEFT SIDEBAR === */}
        <div className="w-[20%] min-w-[200px] hidden md:block border-r border-gray-200">
            <FilterSidebar />
        </div>

        {/* === RIGHT PRODUCT LIST === */}
        <div className="w-full md:w-[80%]">
          <div className="border-b border-gray-200 pb-2 mb-4 shadow-sm sticky top-0 bg-white z-10 flex justify-between items-center">
            <div>
            ...
            {filteredProducts.length > 0 ? (
              filteredProducts.map((item) => (
                ...
                >
                  <Link
                    to={`/product/${item.id}`}
                    className="w-full md:w-64 bg-gray-100 h-64 md:h-52 flex items-center justify-center rounded-md overflow-hidden flex-shrink-0"
                  >
                    <img
                      className="h-full w-full object-contain p-4 hover:scale-105 duration-300 mix-blend-multiply"
                      src={item.image}
                      alt={item.title}
                    />
                  </Link>
                  ...
                  <div className="flex-grow flex flex-col gap-1">
                    ...
                    <div className="text-sm text-gray-600 mt-1">
                      Get it by <span className="font-bold text-gray-800">Tuesday, Aug 25</span>
                    </div>
                  </div>
