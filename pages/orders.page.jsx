...
import { useOrders } from "../src/context/orders.context"; // Import your new context
import moment from "moment"; 

const Orders = () => {
  const { currentUser } = useAuth();
  // Destructure 'loading' instead of 'loadingOrders' to match your Context
              ...
              <div className="bg-gray-100 p-4 flex flex-col md:flex-row justify-between text-sm text-gray-600 rounded-t-md gap-4">
                <div className="flex gap-8">
                    <div>
                        ...
                        <p>{order.createdAt ? moment(order.createdAt.toDate()).format("MMMM Do YYYY") : "N/A"}</p>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <p className="text-blue-600">{currentUser.displayName || currentUser.email}</p>
                    </div>
                </div>
                <div>
                    <p className="uppercase text-xs font-bold">Order # {order.paymentId || order.id}</p>
                </div>
              </div>
